...
class GeneticTrainer(TrainerBase):
    ...
    def __init__(self, env: NewtonBaseEnv, config: GeneticTrainerConfig):
        ...
        self.config = config # For type checker
    ...
    def _init_population(self) -> None:
        ...
        for name, param in self.base_params.items():
            noise = self.config.noise_std * torch.randn(
    ...
    def _compute_actions(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        actions = torch.tanh(actions) * self.config.action_scale
        actions = torch.clamp(actions, -self.config.force_limit, self.config.force_limit)
        return actions
    ...
    def _mutate(self, elite_idx: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        for name, tensor in self.population_params.items():
            ...
            noise = self.config.noise_std * torch.randn_like(trimmed)
    ...
    def train(self) -> None:
        ...
        for gen in range(self.config.generations):
            ...
            for step in range(self.config.episode_steps):
                ...
                # Render
                if self.config.render_every > 0 and gen % self.config.render_every == 0:
                    self.env.render()
            ...
            elite_count = max(1, int(self.config.elite_frac * self.env.num_worlds))
